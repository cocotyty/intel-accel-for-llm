...
class KVShrinkHybridBackendAdapter:
    ...
    def unpersisted_count(self) -> int:
        """Thin forward: entries still memory-only; -1 when unknown."""
        return self._backend.unpersisted_count()

    def chunks_are_persisted(self, group_idx, chunk_labels) -> bool:
        """Thin forward: is every chunk of this boundary durable on disk?
        Callers must treat False as "not committable" (fail closed)."""
        return self._backend.chunks_are_persisted(group_idx,
                                                  list(chunk_labels))
