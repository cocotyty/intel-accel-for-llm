...
class HybridStore:
    ...
    def unpersisted_count(self) -> int:
        """Number of cache entries still held only in memory.

        This is the honest drain condition. ``persist``'s "persisted"
        figure counts GROUPS (kv_pool/mem.cpp dedupes full labels into
        group keys before returning), while its ``max_count`` argument
        limits ENTRIES -- so "persisted < max_count" says nothing about
        whether the queue is empty, and with many layers per group it is
        true on the very first call.

        Returns -1 when the count cannot be obtained, so callers can
        tell "nothing left" apart from "cannot tell" and fail closed.
        """
        if self.flow is None or getattr(self.flow, "mem", None) is None:
            return -1
        return int(self.flow.mem.unpersisted_count)

    def chunks_are_persisted(self, group_idx: int,
                             chunk_labels: List[str]) -> bool:
        """True when every chunk of this boundary is durable on disk.

        A manifest is a promise that some later process can read this
        boundary, and that process can only read what the Record marks
        persisted: ``Mem::get`` falls back to the Storage tier solely
        for persisted groups (kv_pool/mem.cpp), and evicting a group
        that is not yet persisted drops its Record entry outright. So a
        manifest published over memory-only chunks is a promise this
        engine cannot keep once it exits.

        The Record is keyed by GROUP label -- the full chunk label minus
        its trailing ``:<tensor_key>`` component -- which is exactly
        ``_flow_group_label(group_idx) + ':' + <expanded chunk label>``.

        Returns False when there is nothing to consult, so callers fail
        closed (skip the commit -> MISS -> recompute, never bad data).
        """
        if self.flow is None or getattr(self.flow, "record", None) is None:
            return False
        if not chunk_labels:
            return False
        label = self._flow_group_label(group_idx)
        split = self._split_factor(
            self._layout.require_group(group_idx).page_size_bytes)
        group_keys = [f"{label}:{e}"
                      for e in self._expanded_labels(chunk_labels, split)]
        return all(self.flow.record.is_persisted(group_keys))
